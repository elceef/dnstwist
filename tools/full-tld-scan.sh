...
if [ ! -f "$EFFECTIVE_TLD_NAMES" ]; then
     echo "ERROR: Cannot locate file: $EFFECTIVE_TLD_NAMES"
     exit 1
fi

if [ ! -f "$DNSTWIST_SCRIPT" ]; then
...
case $2 in
     gtld)    TLDS="com org net edu info";;
     cctld)   TLDS=$(egrep -o "^[a-z]{2}$" "$EFFECTIVE_TLD_NAMES");;
     *|full)  TLDS=$(egrep -o "^[a-z]{2,}$" "$EFFECTIVE_TLD_NAMES");;
esac
